...
- [x] ✏️ Explain some differences between public and private variables: **Public variables are visible in the Unity Inspector, which allows you to change their values directly from the editor without editing the script. Private variables are hidden from the Unity Inspector and can only be accessed by code within the same script.**
...
- Classes
	- [x] ✏️ Explain the difference between reference and value type variables: **??** Value types (e.g., int, float, bool, structs) store the actual data. Reference types (e.g., class objects, arrays, strings in C#) store a reference (pointer) to the data.
